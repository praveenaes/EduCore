...
// ── Zod schema (mirrors backend teacherValidators) ──────────────────────────────
...
export type CreateTeacherFormValues = z.infer<typeof createSchema>;
export type UpdateTeacherFormValues = z.infer<typeof updateSchema>;
...
interface TeacherFormProps {
  ...
  isLoading?: boolean;
  onSubmit: (
    data: CreateTeacherPayload | Partial<Omit<CreateTeacherPayload, 'employeeId'>>,
    photo: File | null
  ) => Promise<void>;
  onCancel: () => void;
}
...
// ── Component ────────────────────────────────────────────────────────────────
const TeacherForm = forwardRef<HTMLFormElement, TeacherFormProps>(
  ({ mode, defaultValues, isLoading, onSubmit, onCancel }, ref) => {
    ...
    const {
      ...
      defaultValues: {
        ...
        joiningDate: toDateInputValue(defaultValues?.joiningDate),
        qualifications: defaultValues?.qualifications ?? '',
        specializations: defaultValues?.specializations ?? '',
        experience: String(defaultValues?.experience ?? ''),
        salary: String(defaultValues?.salary ?? ''),
        gender: (defaultValues?.gender as 'Male' | 'Female' | 'Other') ?? 'Male',
        dateOfBirth: toDateInputValue(defaultValues?.dateOfBirth),
        bloodGroup: (defaultValues?.bloodGroup as CreateTeacherFormValues['bloodGroup']) ?? 'O+',
    ...
    const handleFormSubmit = async (data: CreateTeacherFormValues | UpdateTeacherFormValues) => {
      const payload = {
        ...data,
      ...
      try {
        if (mode === 'create') {
          await onSubmit(payload as unknown as CreateTeacherPayload, photo);
        } else {
          await onSubmit(
            payload as unknown as Partial<Omit<CreateTeacherPayload, 'employeeId'>>,
            photo
          );
        }
      } catch (err: any) {
        ...
        if (errorsList && Array.isArray(errorsList)) {
          errorsList.forEach((e: any) => {
            ...
            if (fieldName === 'username') {
              setError(mode === 'create' ? 'employeeId' : ('email' as any), {
                type: 'manual',
                message: e.message,
              });
            } else {
    ...
    return (
      <form ref={ref} onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Input
              label="Joining Date"
              type="date"
              error={err.joiningDate?.message}
              {...register('joiningDate')}
            />
            <Input
              label="Qualifications"
              error={err.qualifications?.message}
              {...register('qualifications')}
              placeholder="e.g. M.Ed, B.Sc Physics"
            />
            <Input
              label="Specializations"
              error={err.specializations?.message}
        ...
        <ConfirmationModal
          ...
          message="Are you sure you want to remove this teacher's photo?"
          confirmText="Remove"
          cancelText="Cancel"
          onConfirm={() => {
            handleRemovePhoto();
